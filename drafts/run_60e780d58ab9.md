...
This pattern, sometimes called replay-with-memoization, Temporal pioneered, and most code-first durable execution tools now use. Both Vercel and Cloudflare chose it. That means the execution model is not where they differ.
...
The orchestration the structure implies, not declared explicitly. Nothing in the code announces itself as a workflow engine.
...
The tradeoff is portability versus edge-local state. Neither is the wrong choice. They serve different priorities.
